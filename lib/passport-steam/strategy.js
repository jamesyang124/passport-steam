...
function getUserProfile(key, steamID, callback) {
  ...
  console.log(key);
  console.log("before get player summaries");
...
function Strategy(options, validate) {
  ...
  function verify(req, identifier, profile, done) {
    ...
    // optional chaining support in nodeJS >= 14
    // add package @babel/plugin-proposal-optional-chaining for backward compatibility
    ...
    var steamID = identifierRegex.exec(identifier)[1];
  ...
  this.getUserProfile = getUserProfile.bind(null, options.apiKey);
}
